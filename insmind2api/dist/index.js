...
async function refreshAccounts() {
    try {
        const data = await new Promise((resolve, reject) => {
            const req = http_1.default.get(`${BACKEND_URL}/api/insmind/accounts`, (res) => {
                let d = '';
...
function getNextAccount() {
    ...
    for (let attempt = 0; attempt < accounts.length * 2; attempt++) {
        ...
        const parsed = (0, token_refresh_1.parseTokenProd)(account.token);
 ...
 */
function calculateOssAuth(method, path, headers, sts, date) {
    ...
    const hmac = crypto_1.default.createHmac('sha1', sts.access_key_secret);
...
async function uploadMedia(account, dataUrl) {
    ...
    const sig = crypto_1.default.createHmac('sha1', sts.access_key_secret).update(stringToSign).digest('base64');
    ...
    try {
        result = await new Promise((resolve) => {
            const curl = child_process_1.default.spawn('C:/Windows/System32/curl.exe', [
                '-s', '--max-time', '30',
                '-w', '%{http_code}',
                '-X', 'PUT',
...
async function sseFetch(url, bodyStr, headers, timeoutMs = 300000) {
    try {
...
async function sseFetchStream(url, bodyStr, headers, timeoutMs = 300000) {
    let resp = null;
    try {
...
router.post('/v1/videos/generations', async (ctx) => {
    ...
    try {
        ...
        if (rawData.length > 0)
            console.log(`📄 SSE snippet: ${rawData.substring(0, 500)}`);
    }
    catch (fetchErr) {
                ...
                if (pollResult.status === 200) {
                    ...
                    if (items.length > 0) {
                        ...
                        const videoUrlFromRecord = latest?.generation_result
                            || latest?.assets?.find((a) => a?.url?.includes('.mp4'))?.url
...
router.post('/v1/videos/generations-image', async (ctx) => {
    ...
    if (threadId) {
        ...
        if (rawData && (rawData.includes('"type":"function_call"') || rawData.includes('"type": "function_call"'))) {
            ...
            try {
                const finalResult = await sseFetch('https://sse.insmind.com/api/ai-agent/v1/thread/completion', JSON.stringify(finalPayload), sseHeaders, 300000);
...
app.use((0, koa_body_1.koaBody)({ jsonLimit: '10mb' }));
