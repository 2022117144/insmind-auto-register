...
function getProxyAgent() {
  ...
  if (process.env.HTTPS_PROXY) {
    _proxyAgent = new import_https_proxy_agent.HttpsProxyAgent(process.env.HTTPS_PROXY);
  ...
  for (const port of PORTS) {
    ...
    _proxyAgent = new import_https_proxy_agent.HttpsProxyAgent(url);
...
router.post("/accounts", async (ctx) => {
  const { email, token, userId, refreshToken, orgId } = ctx.request.body;
  if (!email || !token) {
    ctx.status = 400;
...
async function sseFetch(url, bodyStr, headers, timeoutMs = 3e5) {
  try {
    ...
    const resp = await new Promise((resolve, reject) => {
      ...
      req.on("error", (err) => {
        console.log("sseFetch error: " + err.message);
        resolve("");
      });
      req.on("timeout", () => {
        req.destroy();
...
router.post("/v1/videos/pricing", async (ctx) => {
  const { model = "Seedance-2.0-Mini", duration = 5, resolution = "480P" } = ctx.request.body;
  const baseCosts = {
    "Pixverse-V6.0": 50,
    "Wan-2.7": 80,
    "Wan-2.2": 20,
...
router.post("/v1/videos/generations", async (ctx) => {
  ...
  try {
    ...
    ctx.body = {
      id: taskId,
      status: videoUrl ? "success" : "processing",
      model,
      prompt,
      duration,
      resolution,
...
function _guessMime(url) {
  return url.endsWith(".webp") ? "image/webp" : url.endsWith(".jpg") || url.endsWith(".jpeg") ? "image/jpeg" : url.endsWith(".gif") ? "image/gif" : url.endsWith(".png") ? "image/png" : "image/jpeg";
}
router.post("/v1/videos/generations-image", async (ctx) => {
  const { prompt, model = "Seedance-2.0-Mini", duration = 5, resolution = "480P", aspect_ratio = "16:9", image_url, image_urls, account_email } = ctx.request.body;
  const inputUrls = image_urls || (image_url ? [image_url] : []);
  ...
  for (let i = 0; i < inputUrls.length; i++) {
    ...
    if (typeof url === "string" && url.startsWith("data:")) {
      console.log(`\u{1F5BC}\uFE0F [${i + 1}/${inputUrls.length}] Uploading data URL to OSS... (${(url.length / 1024).toFixed(0)}KB)`);
      try {
        cdnUrl = await uploadMedia(account, url);
        console.log(`\u{1F5BC}\uFE0F [${i + 1}/${inputUrls.length}] CDN URL: ${cdnUrl}`);
      } catch (uploadErr) {
  ...
  try {
    ...
    console.log(`\u{1F4E1} SSE response (1st): ${finalRawData.length} chars, model=${model}, resolution=${resolution}, duration=${duration}`);
    console.log(`\u{1F4C4} SSE snippet: ${finalRawData.substring(0, 500)}`);
  } catch (fetchErr) {
    console.error(`\u274C SSE fetch failed: ${fetchErr.message}`);
    ctx.status = 502;
    ctx.body = { error: "SSE request failed", detail: fetchErr.message };
  ...
  if (firstMp4) {
    ...
    ctx.body = {
      ...
      video_url: videoUrl,
      cdn_urls: cdnUrls,
      image_url: image_url || (image_urls ? image_urls[0] : void 0),
  ...
  if (threadId) {
    ...
    if (rawData && (rawData.includes('"type":"function_call"') || rawData.includes('"type": "function_call"'))) {
      console.log(`\u{1F4AC} Forceful final confirmation on thread ${threadId}`);
      ...
      try {
        ...
        console.log(`\u{1F4E1} SSE response (final): ${finalResult.length} chars`);
