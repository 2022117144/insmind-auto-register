...
export default function InsMindAccounts() {
    ...
    const handleAutoRegister = async () => {
        setRegistering(true)
        try {
            const result = await insmindAccountsApi.autoRegister()
            if (result.success) {
    ...
    const handleTestGenerate = async () => {
        setGenerating(true)
        try {
            const result = await insmindAccountsApi.generateVideo({
                prompt: 'A cute cat walking in a sunny park, cinematic quality',
                model: 'Pixverse-V6.0',
                duration: 5,
                resolution: '360P',
                aspect_ratio: '16:9',
            ...
            if (result.success) {
                ...
                toast.error('生成超时（insMind 比较慢），任务可能仍在后台处理')
            } else {
    ...
    const handleBatchDelete = async () => {
        ...
        try {
            const result = await insmindAccountsApi.batchDelete(Array.from(selectedIds))
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button
                        ...
                        disabled={generating}
                        className="h-9 border-purple-500/30 text-purple-400 hover:bg-purple-500/10 rounded-[10px]"
                    >
                        {generating ? (
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        ) : (
