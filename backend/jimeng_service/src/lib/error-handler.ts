...
function buildDetailedApiError(response: JimengErrorResponse): string {
  const { ret, errmsg, data } = response;
  const failCode = data?.fail_code;
  const failStarlingKey = data?.fail_starling_key;
  const failStarlingMessage = data?.fail_starling_message;
  const normalizedMessage = `${errmsg || ""} ${failStarlingMessage || ""}`.toLowerCase();

  if (
    normalizedMessage.includes("shark") ||
    normalizedMessage.includes("not pass reject") ||
    normalizedMessage.includes("risk not pass")
  ) {
    return [
      `[上游安全中间件拦截]: ${failStarlingMessage || errmsg}`,
      failStarlingKey ? `starling_key=${failStarlingKey}` : null,
      failCode ? `fail_code=${failCode}` : null,
      "建议：更换账号、代理区域、提示词或参考素材，必要时先在官方页面完成人工验证后再重试。",
    ].filter(Boolean).join(" ");
  }
  ...
  if (ret === "4010") {
    return [
      `[视频合规确认未完成]: ${errmsg}`,
      failStarlingKey ? `starling_key=${failStarlingKey}` : null,
      "建议：先在网页版视频生成页同意确认弹窗，或使用已集成的自动确认逻辑后重试。",
    ].filter(Boolean).join(" ");
  }

  if (ret === "4001") {
    const message = failStarlingMessage || errmsg;
    const isReferenceRelated = /face|reference|material|subject|upload/i.test(
      `${failStarlingKey || ""} ${message || ""}`,
    );
    if (isReferenceRelated) {
      return [
        ...
        failStarlingKey ? `starling_key=${failStarlingKey}` : null,
        failCode ? `fail_code=${failCode}` : null,
        "建议：更换参考图/参考视频，避免真人敏感面部、未授权肖像或高风险内容。",
      ].filter(Boolean).join(" ");
    }
    return [
      `[提示词或内容审核未通过]: ${message}`,
...
export class JimengErrorHandler {
  ...
  static handleApiResponse(
    ...
    switch (ret) {
      ...
      case '4001':
        throw new APIException(EX.API_CONTENT_FILTERED, buildDetailedApiError(response));
      ...
      case '5002':
        throw new APIException(EX.API_VIDEO_GENERATION_FAILED, buildDetailedApiError(response));

      case '4010':
        throw new APIException(EX.API_VIDEO_GENERATION_FAILED, buildDetailedApiError(response));

      default:
        throw new APIException(EX.API_REQUEST_FAILED, buildDetailedApiError(response));
    }
  }
